...
from app.config import (directories, resume_logging_to_console,
                        stop_logging_to_console)

from ..db.crud import (bulk_load_action_histories, bulk_load_complaints,
                       bulk_load_districts, filter_api_request,
                       get_all_complaints, mark_api_request_failed,
    ...
    try:
        ...
        if ingest_action_history:
            try:
                ...
                logger.info(
                    f"Processing action history for {len(complaints)} complaints"
                )
                action_tasks = [
