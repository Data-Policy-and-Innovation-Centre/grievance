...
def test_update_document_status_updates_existing_fields(db_session, sample_complaint_data):
...
def test_update_document_status_preserves_other_fields(db_session, sample_complaint_data):
...
def test_update_document_status_multiple_complaints(db_session, sample_complaint_data):
    ...
    result2 = update_document_status(
        db_session, "T456", None, False, "Download failed"
    )
...
def test_update_document_status_database_commit(db_session, sample_complaint_data):
    ...
    from app.db.crud import create_or_update_complaint, update_document_status, get_complaint_by_ticket
...
def test_update_document_status_error_handling(db_session):
    ...
    try:
        ...
        assert "database" in str(e).lower() or "connection" in str(e).lower() or "operationalerror" in str(e).lower()
...
def test_update_document_status_edge_cases(db_session, sample_complaint_data):
    ...
    result1 = update_document_status(
        db_session, "T123", long_path, True, None
    )
    ...
    result3 = update_document_status(
        db_session, "T123", None, False, None
    )
...
def test_update_document_status_integration_with_get_complaints_without_documents(
    ...
    update_document_status(
        db_session, "T123", "/path/to/document.pdf", True, None
    )
    ...
    update_document_status(
        db_session, "T123", None, False, "Download failed"
    )
    
    # Complaint should be back in the list (not downloaded)
    complaints_failed = get_complaints_without_documents(db_session, get_docs_where_errors_occurred=True)
