...
async def test_download_document_no_path_logs_and_updates_status(doc_service):
    ...
    with patch.object(doc_service, "get_document_path", return_value=None), \
...
async def test_batch_download_documents_handles_exception(doc_service, db_session):
    ...
    with patch.object(doc_service, "download_document", side_effect=Exception("Boom!")), \
         patch("app.ingestion.document_ingestion.update_document_status") as mock_update_status, \
         patch("app.ingestion.document_ingestion.tqdm"):  # para evitar output en test
...
@pytest.mark.asyncio
async def test_update_document_status_for_all_complaints_without_documents(doc_service, db_session, tmp_path, monkeypatch):
    """Test update_document_status_for_all_complaints when only_without_documents is True"""
    from app.db.crud import get_complaints_without_documents, update_document_status
    from app.config import settings
