...
class TestJansunaniAPIClient:
    ...
    async def test_get_complaints_success(
        self, mock_async_client, client: JanasunaniAPIClient
    ):
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
        ...
        result = await client.get_complaints(
            year=2024, distId=1, status=2, office=4, semaphore=semaphore
        )
    ...
    async def test_get_complaints_wrong_params(self, client: JanasunaniAPIClient):
        with pytest.raises(ValueError, match="Status must be in"):
            await client.get_complaints(
                year=2024, distId=1, status=3, office=4, semaphore=asyncio.Semaphore(5)
            )
        with pytest.raises(ValueError, match="Office must be in"):
            await client.get_complaints(
                year=2024, distId=1, status=2, office=8, semaphore=asyncio.Semaphore(5)
            )

    @patch("app.ingestion.client.httpx.Client")
    def test_handle_response_missing_keys(
        self, mock_client, client: JanasunaniAPIClient
    ):
        mock_response = MagicMock()
        mock_response.status_code = 200
        ...
        with pytest.raises(
            JanasunaniAPIError,
            match="Neither 'distRes', 'Res' or 'actionHistory' found in response.",
        ):
            client.get_districts()

    @patch("app.ingestion.client.httpx.Client")
    def test_handle_response_api_error_status(
        self, mock_client, client: JanasunaniAPIClient
    ):
        mock_response = MagicMock()
        mock_response.status_code = 200
        ...
        with pytest.raises(
            JanasunaniAPIError, match="Jansunani API returned an error: Bad Request"
        ):
            client.get_districts()
    ...
    @pytest.mark.asyncio
    @patch("app.ingestion.client.httpx.AsyncClient")
    async def test_get_action_history_success(
        self, mock_async_client, client: JanasunaniAPIClient
    ):
        mock_response = MagicMock()
        mock_response.status_code = 200
    ...
    @pytest.mark.asyncio
    @patch("app.ingestion.client.httpx.AsyncClient")
    async def test_handle_response_action_history(
        self, mock_async_client, client: JanasunaniAPIClient
    ):
        mock_response = MagicMock()
        mock_response.status_code = 200
