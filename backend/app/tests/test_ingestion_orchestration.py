...
from app.db.models import Base, District as DistrictModel
...
class TestIngestionOrchestrator:
    ...
    def test_ingest_districts_success(self, mock_validate, mock_bulk_load, orchestrator, sample_district_data):
        """Test successful district ingestion."""
        # Mock client response
        orchestrator.client.get_districts = MagicMock(return_value=sample_district_data)
        
        # Mock validation
        validated_districts = [District(**district) for district in sample_district_data]
        ...
        mock_validate.assert_called_once_with(sample_district_data, District, dict_mode=False)
    ...
    def test_ingest_districts_client_error(self, mock_logger, orchestrator):
        ...
        orchestrator.client.get_districts = MagicMock(side_effect=Exception("API Error"))
    ...
    @patch('app.ingestion.orchestrator.validate')
    @pytest.mark.asyncio
    async def test_ingest_complaints_success(self, mock_validate, mock_bulk_load, orchestrator, sample_complaint_data):
        """Test successful complaint ingestion."""
        # Mock client response
        orchestrator.client.get_complaints = AsyncMock(return_value=sample_complaint_data)
        
        # Mock validation
        validated_complaints = [Complaint(**complaint) for complaint in sample_complaint_data]
        ...
        orchestrator.client.get_complaints.assert_called_once_with(2024, 1, 1, 1, orchestrator.semaphore)
        mock_validate.assert_called_once_with(sample_complaint_data, Complaint, dict_mode=False)
    ...
    async def test_ingest_complaints_client_error(self, orchestrator):
        ...
        # Mock client to raise exception
        orchestrator.client.get_complaints = AsyncMock(side_effect=Exception("API Error"))
    ...
    @pytest.mark.asyncio
    async def test_ingest_action_history_success(self, mock_record_success, mock_validate, mock_bulk_load, orchestrator, sample_action_history_data):
        """Test successful action history ingestion."""
        # Mock client response
        orchestrator.client.get_action_history = AsyncMock(return_value=sample_action_history_data)
        
        # Mock validation
        validated_history = [ActionHistory(**action) for action in sample_action_history_data]
        ...
        orchestrator.client.get_action_history.assert_called_once_with("T123", orchestrator.semaphore)
        mock_validate.assert_called_once_with(items=sample_action_history_data, ticket_no="T123", dict_mode=False)
        mock_bulk_load.assert_called_once_with(orchestrator.db, validated_history)
        mock_record_success.assert_called_once_with(orchestrator.db, "T123", len(sample_action_history_data))
    ...
    async def test_ingest_action_history_none_response(self, orchestrator):
        ...
        with patch('app.ingestion.orchestrator.mark_action_history_api_request_failed') as mock_mark_failed:
            # Execute
            result = await orchestrator.ingest_action_history("T123")
    ...
    async def test_ingest_action_history_client_error(self, orchestrator):
        ...
        # Mock client to raise exception
        orchestrator.client.get_action_history = AsyncMock(side_effect=Exception("API Error"))
    ...
    async def test_ingest_documents_success(self, orchestrator, sample_complaint_data):
        ...
        orchestrator.doc_service.batch_download_documents = AsyncMock(return_value=expected_results)
        ...
        orchestrator.doc_service.batch_download_documents.assert_called_once_with(complaints)
...
class TestOrchestratorIntegration:
    ...
    @patch('app.ingestion.orchestrator.validate')
    @pytest.mark.asyncio
    async def test_full_ingestion_workflow(self, mock_validate, mock_bulk_load_complaints, mock_bulk_load_districts, db_session, sample_district_data, sample_complaint_data):
        ...
        orchestrator.doc_service.batch_download_documents = AsyncMock(return_value={"T123": "success"})
