...
class TestRunIngestionService:
    ...
    async def test_run_ingestion_service_complaints_only(
        self, mock_resume_logging, mock_stop_logging, mock_mark_failed, 
        mock_record_success, mock_filter, mock_get_db, db_session, sample_district_data, sample_complaint_data
    ):
        ...
        with patch('app.ingestion.orchestrator.IngestionOrchestrator') as mock_orchestrator_class:
            ...
            mock_orchestrator.ingest_complaints = AsyncMock(return_value=sample_complaint_data)
    ...
    async def test_run_ingestion_service_documents_only(
        self, mock_resume_logging, mock_stop_logging, mock_get_complaints, 
        mock_get_db, db_session, sample_district_data, sample_complaint_data
    ):
        ...
        complaints = [ComplaintModel(**complaint) for complaint in sample_complaint_data]
        ...
        with patch('app.ingestion.orchestrator.IngestionOrchestrator') as mock_orchestrator_class:
            ...
            mock_orchestrator.ingest_documents = AsyncMock(return_value={"T123": "success"})
    ...
    async def test_run_ingestion_service_action_history_only(
        self, mock_resume_logging, mock_stop_logging, mock_get_tickets, 
        mock_get_db, db_session, sample_district_data, sample_action_history_data
    ):
        ...
        with patch('app.ingestion.orchestrator.IngestionOrchestrator') as mock_orchestrator_class:
            ...
            action_history = [ActionHistory(**action) for action in sample_action_history_data]
            mock_orchestrator.ingest_action_history = AsyncMock(return_value=action_history)
    ...
    async def test_run_ingestion_service_mixed_success_failure(
        ...
        with patch('app.ingestion.orchestrator.IngestionOrchestrator') as mock_orchestrator_class:
            ...
            mock_orchestrator.ingest_complaints = AsyncMock(side_effect=[
    ...
    async def test_run_ingestion_service_no_districts_in_db(
        self, mock_resume_logging, mock_stop_logging, mock_filter, mock_get_db, db_session, sample_district_data
    ):
    ...
    async def test_run_ingestion_service_all_components(
        self, mock_resume_logging, mock_stop_logging, mock_mark_failed, 
        mock_record_success, mock_filter, mock_get_db, db_session, 
        sample_district_data, sample_complaint_data, sample_action_history_data
    ):
        ...
        with patch('app.ingestion.orchestrator.get_complaints_without_documents') as mock_get_complaints, \
             patch('app.ingestion.orchestrator.get_tickets_needing_action_history') as mock_get_tickets:
    ...
    async def test_run_ingestion_service_exception_handling(
        ...
        with patch('app.ingestion.orchestrator.IngestionOrchestrator') as mock_orchestrator_class:
            ...
            mock_orchestrator.ingest_complaints = AsyncMock(side_effect=Exception("Ingestion failed"))
            ...
            # Verify
            assert result["statusCode"] == 200  # Service should continue despite individual failures
            assert "Data ingestion completed successfully" in result["body"]
