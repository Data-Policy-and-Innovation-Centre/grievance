...
def s3_service(mock_s3_client, mock_s3_resource):
    ...
    return S3Service(bucket_name="test-bucket", s3_client=mock_s3_client, s3_resource=mock_s3_resource)
...
class TestS3Service:
    ...
    def test_upload_file_client_error(self, s3_service, mock_s3_client, tmp_path):
        ...
        with patch.object(mock_s3_client, 'upload_file', side_effect=ClientError(
    ...
    def test_upload_file_no_credentials(self, s3_service, mock_s3_client, tmp_path):
        ...
        with patch.object(mock_s3_client, 'upload_file', side_effect=NoCredentialsError()):
            result = s3_service.upload_file(
                file_path=str(test_file),
                s3_key="documents/test.pdf"
            )
    ...
    def test_upload_fileobj_client_error(self, s3_service, mock_s3_client):
        ...
        with patch.object(mock_s3_client, 'upload_fileobj', side_effect=ClientError(
    ...
    def test_download_file_client_error(self, s3_service, mock_s3_client):
        ...
        with patch.object(mock_s3_client, 'download_file', side_effect=ClientError(
    ...
    def test_download_file_no_credentials(self, s3_service, mock_s3_client):
        ...
        with patch.object(mock_s3_client, 'download_file', side_effect=NoCredentialsError()):
            result = s3_service.download_file(
                s3_key="documents/test.pdf",
                local_path="/tmp/test.pdf"
            )
    ...
    def test_get_object_client_error(self, s3_service, mock_s3_client):
        ...
        with patch.object(mock_s3_client, 'get_object', side_effect=ClientError(
            {'Error': {'Code': 'NoSuchKey', 'Message': 'Object does not exist'}},
            'get_object'
        )):
            result = s3_service.get_object("documents/test.pdf")
    ...
    def test_list_objects_success(self, s3_service, mock_s3_client):
        ...
        with patch.object(mock_s3_client, 'list_objects_v2', return_value=mock_response):
    ...
    def test_list_objects_client_error(self, s3_service, mock_s3_client):
        ...
        with patch.object(mock_s3_client, 'list_objects_v2', side_effect=ClientError(
            {'Error': {'Code': 'AccessDenied', 'Message': 'Access denied'}},
            'list_objects_v2'
        )):
            result = s3_service.list_objects()
    ...
    def test_list_objects_no_credentials(self, s3_service, mock_s3_client):
        ...
        with patch.object(mock_s3_client, 'list_objects_v2', side_effect=NoCredentialsError()):
            result = s3_service.list_objects()
    ...
    def test_delete_object_client_error(self, s3_service, mock_s3_client):
        ...
        with patch.object(mock_s3_client, 'delete_object', side_effect=ClientError(
            {'Error': {'Code': 'NoSuchKey', 'Message': 'Object does not exist'}},
            'delete_object'
        )):
            result = s3_service.delete_object("documents/test.pdf")
    ...
    def test_delete_object_no_credentials(self, s3_service, mock_s3_client):
        ...
        with patch.object(mock_s3_client, 'delete_object', side_effect=NoCredentialsError()):
            result = s3_service.delete_object("documents/test.pdf")
    ...
    def test_object_exists_false_404(self, s3_service, mock_s3_client):
        ...
        with patch.object(mock_s3_client, 'head_object', side_effect=ClientError(
    ...
    def test_object_exists_no_credentials(self, s3_service, mock_s3_client):
        ...
        with patch.object(mock_s3_client, 'head_object', side_effect=NoCredentialsError()):
            result = s3_service.object_exists("documents/test.pdf")
    ...
    def test_get_presigned_url_success(self, s3_service, mock_s3_client):
        ...
        with patch.object(mock_s3_client, 'generate_presigned_url', return_value="https://test-bucket.s3.amazonaws.com/documents/test.pdf?signature=abc123"):
            result = s3_service.get_presigned_url("documents/test.pdf", expiration=7200)
            
            assert result == "https://test-bucket.s3.amazonaws.com/documents/test.pdf?signature=abc123"
    ...
    def test_get_presigned_url_client_error(self, s3_service, mock_s3_client):
        ...
        with patch.object(mock_s3_client, 'generate_presigned_url', side_effect=ClientError(
            {'Error': {'Code': 'NoSuchKey', 'Message': 'Object does not exist'}},
    ...
    def test_get_presigned_url_no_credentials(self, s3_service, mock_s3_client):
        ...
        with patch.object(mock_s3_client, 'generate_presigned_url', side_effect=NoCredentialsError()):
            result = s3_service.get_presigned_url("documents/test.pdf")
    ...
    def test_integration_workflow(self, s3_service, mock_s3_client, tmp_path):
        ...
        with patch.object(mock_s3_client, 'generate_presigned_url', return_value="https://test-bucket.s3.amazonaws.com/documents/test.pdf?signature=abc123"):
            ...
            assert result3 == "https://test-bucket.s3.amazonaws.com/documents/test.pdf?signature=abc123"
...
class TestS3ServiceErrorHandling:
    ...
    def test_upload_file_with_nonexistent_file(self, s3_service, mock_s3_client):
        ...
        with patch.object(mock_s3_client, 'upload_file', side_effect=FileNotFoundError("No such file or directory")):
            result = s3_service.upload_file(
    ...
    def test_download_file_with_nested_path(self, s3_service, mock_s3_client, tmp_path):
        ...
        result = s3_service.download_file(
            s3_key="documents/2025/january/test.pdf",
            local_path=str(download_path)
        )
    ...
    def test_object_exists_with_special_characters(self, s3_service, mock_s3_client, tmp_path):
