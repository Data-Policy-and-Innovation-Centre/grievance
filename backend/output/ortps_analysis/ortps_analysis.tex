...
The final analysis dataset comprises \textbf{721,047 English-language complaints} from July 2023 onwards.
...
Across the 8 updated categories, total ORTPS-identified complaints increased from \textbf{12,796 in FY2023} to \textbf{23,744 in FY2024} (\textbf{+85.6\%}).

Key cross-sectional patterns are:

\begin{itemize}
    \item \textbf{Ration card} remains the largest category by a wide margin (8,522 $\rightarrow$ 14,620 complaints), with share remaining high and stable-to-rising (3.14\% in FY2023 vs 3.25\% in FY2024). This indicates persistent high demand for ration-card service delivery.

    \item \textbf{Certificates} is the second-largest category (1,770 $\rightarrow$ 3,642; +105.8\%), with share increasing from 0.65\% to 0.81\%. Certificate-linked grievances are both high-volume and growing rapidly.

    \item \textbf{Land matters} shows the fastest relative growth (512 $\rightarrow$ 2,028; nearly 4x), with share rising from 0.19\% to 0.45\%. This remains the sharpest year-on-year expansion among the major categories.

    \item \textbf{Scholarship} also grows materially in absolute terms (1,045 $\rightarrow$ 1,837; +75.8\%) while share remains stable-to-rising (0.38\% to 0.41\%), suggesting sustained demand that scales with overall complaint growth.
...
\subsubsection{Ration card (FY2023: 8,522; FY2024: 14,620)}

Topic modeling on 12,125 complaints (24 topics; 82.7\% modeled coverage) shows that the largest single topic is explicitly \textbf{ration-card service requests} (Topic 0: 4,307 complaints; 35.5\%), with keywords centered on ``ration card'', ``new ration card'', ``number'', and ``add''. Housing-related spillover remains visible in secondary topics (e.g., Topic 1: 778 complaints; 6.4\%, Topic 3: 525 complaints; 4.3\%).
...
\textbf{Assessment:} The category now has a strong core ORTPS signal, with authentic ration-card delivery bottlenecks dominating. However, non-ORTPS housing petitions and template submissions still contribute measurable noise.

\subsubsection{Certificates (FY2023: 1,770; FY2024: 3,642)}

Topic modeling on 4,585 complaints (21 topics; 93.2\% modeled coverage) is led by certificate-service themes: Topic 0 (920 complaints; 20.1\%) centers on caste/SEBC/OBC certificates, Topic 1 (389 complaints; 8.5\%) on legal heir certificates, and Topic 3 (330 complaints; 7.2\%) on birth-certificate workflows. Housing-related themes still appear in secondary topics (e.g., Topic 2: 361 complaints; 7.9\%).
...
\textbf{Assessment:} Certificate-service content is now clearly dominant, with strong clusters for caste, legal-heir, and birth/death workflows. The category is still mixed, but off-topic housing noise is now secondary rather than primary.

\subsubsection{Land matters (FY2023: 512; FY2024: 2,028)}

Topic modeling on 307 complaints (2 topics; 92.8\% modeled coverage) is highly concentrated: Topic 0 (236 complaints; 76.9\%) is strongly land-service-specific with ``land/ror/plot/mutation/registration'' language, while Topic 1 (49 complaints; 16.0\%) captures PMAY-style housing spillover.
...
\textbf{Assessment:} Land matters now shows much stronger thematic purity, with most modeled complaints tied to mutation/RoR/registration processes. Residual inflation remains from PMAY-style spillover and template submissions.

\subsubsection{Scholarship (FY2023: 1,045; FY2024: 1,837)}

Topic modeling on 2,571 complaints (8 topics; 86.6\% modeled coverage) is dominated by scholarship workflows: Topic 0 (1,756 complaints; 68.3\%) centers on scholarship sanction/disbursement with strong labour-card-linked scholarship patterns; Topic 1 (102 complaints; 4.0\%) captures Odia-language payment-delay complaints.
...
\textbf{Assessment:} Scholarship grievances are now the dominant category signal, especially around pending sanction, delayed disbursement, and profile/eligibility frictions. Spillover from adjacent schemes still appears in smaller topics.

\subsection{Cross-Cutting Pattern in the Four Focus Categories}

Across all four categories, a common secondary pattern persists: \textbf{PM Awas-style housing templates recur as non-trivial spillover topics}, especially in Ration card, Certificates, and Land matters. However, unlike earlier runs, the dominant topic in three of the four categories is now service-specific (Ration card, Certificates, Scholarship).
...
For these four categories, reported volumes in Table~\ref{tab:ortps_aggregation} are now materially closer to true service demand, but should still be treated as \emph{upper-bound estimates} unless the pipeline explicitly filters:
\begin{enumerate}
...
\textbf{Note:} Scholarship complaints show consistent 75.8\% growth from FY2023-24. Word clouds may reveal seasonal patterns (academic calendar dependencies), common delays in disbursement, or eligibility verification issues. Terms related to deadlines and urgency are likely prominent.
...
\textbf{Note:} As the dominant ORTPS category with 71.6\% FY2023-24 growth, Ration Card word clouds are critical for understanding systemic service delivery challenges. Expect high frequency of terms related to updates, corrections, additions/deletions of members, and digitization issues (e.g., Aadhaar linking).
